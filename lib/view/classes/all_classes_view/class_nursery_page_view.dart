import 'package:flutter/material.dart';
import 'package:get/get.dart';
import '../all_classes_controller/all_classes_controller.dart';
import '../../../widgets/class_page_item_view.dart';
import '../../../widgets/every_class_appbar.dart';

class ClassNurseryPageView extends StatefulWidget {
  const ClassNurseryPageView({super.key});

  @override
  State<ClassNurseryPageView> createState() => _ClassNurseryPageViewState();
}

class _ClassNurseryPageViewState extends State<ClassNurseryPageView> {
  @override
  Widget build(BuildContext context) {
    var controller = Get.find<AllClassesController>();
    return Scaffold(
      appBar: EveryClassAppbar(className: 'Nursery'),
      body: GetBuilder<AllClassesController>(
          init: controller.getAllStudentByClassName('nursery'),
          builder: (context) {
            return ClassPageItemView(controller: controller);
          }),
    );
  }
}
